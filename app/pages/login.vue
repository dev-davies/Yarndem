    ...
    <div class="max-w-md w-full space-y-8 bg-yarn-cream/50 p-8 md:p-12 rounded-3xl border border-yarn-black/5 shadow-2xl shadow-yarn-black/5 backdrop-blur-sm">
      <div class="text-center">
        ...
        <h2 class="text-3xl font-display font-medium text-yarn-black tracking-tight">
          {{ isRegistering ? 'Create your vault' : 'Welcome back' }}
        </h2>
        <p class="mt-3 text-sm text-yarn-black/50 font-light">
          {{ isRegistering ? 'Set up your secure YarnDem account' : 'Access your secure YarnDem vault' }}
        </p>
      </div>
      ...
      <form class="mt-10 space-y-6" @submit.prevent="handleLogin">
        ...
        <div class="text-center space-y-4">
          <button
            type="button"
            :disabled="isLoading"
            class="text-xs text-yarn-black/60 hover:text-yarn-terracotta transition-colors duration-200 disabled:opacity-50"
            @click="toggleMode"
          >
            <template v-if="isRegistering">
              Already have an account? <span class="text-yarn-terracotta font-medium">Login</span>
            </template>
            <template v-else>
              New here? <span class="text-yarn-terracotta font-medium">Create an account</span>
            </template>
          </button>
...
const buttonLabel = computed(() => {
  if (isLoading.value) {
    return isRegistering.value ? 'Generating Keys…' : 'Decrypting…'
  }
  return isRegistering.value ? 'Generate Keys & Register' : 'Login & Decrypt'
})
